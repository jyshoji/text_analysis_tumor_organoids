...
### Changing the column name so that an existing custom function can work without further changes.
tumor_types_A <- tumor_types_F %>% 
...
identified_total <- tumor_types_A %>% 
...
### Writing a (csv) file for text position adjustments
### The csv file will be used to manually adjust the positions to avoid text overlapping.
...
### Plotting for tumor organoids.
tumor_edge_lvl3 <- fn_edge_lvl3(combined_trends, edge_all[, 1:2], corpus_type = "tumor_organoid")

tumor_organ_lvl3_count <- fn_count_lvl3(combined_trends, tumor_edge_lvl3, corpus_type = "tumor_organoid")

tumor_nudge <- fn_write_nudge_positions(tumor_edge_lvl3, position_adjustment = TRUE)
...
tumor_bubble <- fn_celtic_circles(tumor_edge_lvl3, tumor_organ_lvl3_count, tumor_nudge, random_seed = 1) 


ggsave(tumor_bubble, 
       filename = paste0(root_path, "results/organ_classifications/tumor_bubble.pdf"),  
       device = cairo_pdf, 
...
### ToC
###
ToC_edge_lvl3 <- fn_edge_lvl3(combined_trends, edge_all[, 1:2], corpus_type = "ToC")

ToC_organ_lvl3_count <- fn_count_lvl3(combined_trends, ToC_edge_lvl3, corpus_type = "ToC")

ToC_nudge <- fn_write_nudge_positions(ToC_edge_lvl3, position_adjustment = TRUE)
### If you do not want to manually adjust the positions of text labels, change "position_adjustment = TRUE" to 
### "position_adjustment = FALSE".
### Also, delete the following line to read csv.
...
ToC_bubble <- fn_celtic_circles(ToC_edge_lvl3, ToC_organ_lvl3_count, ToC_nudge, random_seed = 2) 


ggsave(ToC_bubble, 
       filename = paste0(root_path, "results/organ_classifications/ToC_bubble.pdf"),  
       device = cairo_pdf, 
       width = 178, height = 178, units = "mm")
...
fn_organ_network <- function(vertice_data, edge_list) {
  ...
  corpus_type_converted <- ifelse(corpus_type == "tumor_organoid", "tumor organoids", 
                                  ifelse(corpus_type %in% c("organoid", "OoC", "ToC"), paste0(corpus_type, "s"), NA))
...
### Drawing network graph of tumor organoid
tumor_counts_SP <- fn_count_SP(combined_trends, corpus_type = "tumor_organoid")

tumor_edge_SP <- fn_edge_SP(tumor_counts_SP, edge_all[, 1:2])

tumor_network <- fn_organ_network(tumor_counts_SP, tumor_edge_SP) + 
...
ggsave(tumor_network, 
       filename = paste0(root_path, "results/organ_classifications/tumor_network.pdf"), 
       device = cairo_pdf, 
...
### Drawing network graph of ToC.
ToC_counts_SP <- fn_count_SP(combined_trends, corpus_type = "ToC")

ToC_edge_SP <- fn_edge_SP(ToC_counts_SP, edge_all[, 1:2])

ToC_network <- fn_organ_network(ToC_counts_SP, ToC_edge_SP) + 
  scale_size_continuous(limits = c(0, sqrt(300)), breaks = c(sqrt(10), sqrt(100)), labels = c(10, 100)) + 
  theme(
...
ggsave(ToC_network, 
       filename = paste0(root_path, "results/organ_classifications/ToC_network.pdf"), 
       device = cairo_pdf, 
